...
app = FastAPI(
    title="ArogyaMitr MVP Core",
    description="Decentralized Dual-Outlet Rural Healthcare & Voice-Guided Tele-Care Platform (SIH 2026 PS 26133)",
    version="2.0.0"
)
...
# In-Memory Database for Live Demonstration & Local Testing
DB = {
    "stats": {
        "today_triages": 28,
    ...
    "prescriptions": [
        {
            ...
            "status": "Dispatched to Doorstep (Runner ETA: 14 Mins)",
...
def create_triage(data: TriageInput):
    # Automated Clinical Risk Scoring Engine
    risk_level = "Normal"
    reasons = []
...
def issue_prescription(data: PrescriptionInput):
    ...
    record = {
        ...
        "status": "Packed at Dark Hub with Dosage Stickers • Out for Home Delivery (ETA: 18 Mins)",
        "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat()
    }
